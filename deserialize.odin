...
process_initial :: proc(using partial: Partial_Packet, packet: []u8) -> (Packet, []u8, Transport_Error) {
	...
	secrets := conn.encryption.secrets[.ssl_encryption_initial]
	switch s in secrets {
	case TLS_Secret:
	    hp_key = s.hp
	case Initial_Secret:
...
process_handshake :: proc(using partial: Partial_Packet, packet: []u8) -> (Packet, []u8, Transport_Error) {
    packet := packet
    
    hp_key: []byte
    conn := find_conn(dest_conn_id)
    if conn != nil { // FIXME: REPLACE WITH ZII
	hp_key = get_hp_key(conn, .ssl_encryption_handshake)
    } else {
...
process_retry :: proc(using partial: Partial_Packet, packet: []u8) -> (Packet, []u8, Transport_Error) {
...
process_version_negotiation :: proc(using partial: Partial_Packet, packet: []u8) -> (Packet, []u8, Transport_Error) {
    packet := packet
...
process_zero_rtt :: proc(using partial: Partial_Packet, packet: []u8) -> (Packet, []u8, Transport_Error) {
    packet := packet

    hp_key: []byte
    conn := find_conn(dest_conn_id);
    if conn != nil { // FIXME: REPLACE WITH ZII
	hp_key = get_hp_key(conn, .ssl_encryption_early_data)
    } else {
	return nil, nil, .PROTOCOL_VIOLATION // We can't find the conn object for the handshake so... nah
    }
...
process_one_rtt :: proc(partial: Partial_Packet, packet: []u8) -> (Packet, []u8, Transport_Error) 
{
    ...
    conn := find_conn(dest_conn_id)
    if conn != nil { // FIXME: REPLACE WITH ZII
	hp_key = get_hp_key(conn, .ssl_encryption_application)
    } else {
	return nil, nil, .PROTOCOL_VIOLATION // We can't find the conn object for the handshake so... nah
    }
    ...
    packet_number: u32
    decrypted_first_byte, packet_number, packet = remove_header_protection(first_byte, packet, mask)
